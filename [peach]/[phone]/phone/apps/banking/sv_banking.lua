...
Phone.API.RegisterServerEvent('bank:transfer', function(src, target, amt)
  ...
  if tonumber(bank) >= amt then
    ...
    TriggerClientEvent('phone:notify', src, { app = 'banking', title = Locales.bankingTitle, content = 'Has transferido $' .. amt .. ' Para #' .. target, sameApp = true })
    TriggerClientEvent('phone:notify', target, { app = 'banking', title = Locales.bankingTitle, content = '#' .. src .. ' te ha enviado $' .. amt, sameApp = true })
  else
