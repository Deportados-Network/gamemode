    ...
    while true do
        ...
        if nearbyObject and IsPedOnFoot(playerPed) then
            ...
            if IsControlJustReleased(0, 38) and not isPickingUp then
                ...
                local policeOnline = 2

                for _, player in ipairs(GetActivePlayers()) do
                    local targetPed = GetPlayerPed(player)
                    local targetJob = ESX.GetPlayerData(player).job

                    if targetJob and targetJob.name == 'police' and IsPedOnFoot(targetPed) then
                ...
                if policeOnline >= 2 then
                    ...
                    ESX.ShowNotification("No hay suficientes policías en línea para farmear marihuana.")
                    isPickingUp = false
                end
            end
        end
