...
Radio.Labels = {        
	...
	{ "FRZL_RADIO_INPUT", "Frecuencia" },
}

-- Define las teclas
Keys = {
    ['SPACE'] = 22, -- Puedes cambiar el valor según tu configuración
    -- Agrega otras teclas según sea necesario
}
...
Radio.Commands = {
	{
		...
		Help = "Para abir la radio", -- Command help shown in chatbox when typing the command
		Params = {},
		Handler = function(src, args, raw)
			ESX.TriggerServerCallback('rp-radio:hasRadio', function(has)
				if (has) then
...
function Radio:Toggle(toggle)
	...
	if not self.Has or isDead then
		self.Open = false
		self.On = false
		...
		if DoesEntityExist(self.Handle) then
			...
			while not NetworkHasControlOfEntity(self.Handle) do
				Citizen.Wait(0)
			end
...
CreateThread(function()
	Wait(3500)
	local mafias = {}
	ESX.TriggerServerCallback('bm_mafias:getAll', function(vtable)
		mafias = vtable
		for i, v in pairs(vtable) do
			local mainId = 100
			AddPrivateFrequency(mainId + v.id)
			print("Private frequency: " .. mainId + v.id)
		end
	end)
	for i = 1, 11 do
		AddPrivateFrequency(i)
	end
	Wait(1000)
	while true do
		local inMafia, name, data = exports['bm_mafias']:isPlayerMafia()

		local mainId = 100
		if inMafia then
			GivePlayerAccessToFrequency(mainId + data.id)
		else
			for i, v in pairs(mafias) do
				RemovePlayerAccessToFrequency(mainId + v.id)
			end
		end
		...
		if jobName and jobName == "police" then
			for i = 1, 24 do
				GivePlayerAccessToFrequency(i)
			end
		else
			for i = 1, 24 do
				RemovePlayerAccessToFrequency(i)
			end
		end
    ...
    while true do
        ...
        if isBroadcasting then
            if IsControlPressed(0, Keys['SPACE']) then
                -- Si se está hablando por radio y la tecla de espacio está presionada, desactívala
                DisableControlAction(0, Keys['SPACE'], true)
                msec = 0
            end
        else
            -- Si no se está transmitiendo por radio, habilita la tecla de espacio
...
RegisterNetEvent('rp-radio:use', function()
	Radio:Toggle(not Radio.Open)
end)