...
RegisterCommand('checheche', function()
    local playerPed = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(playerPed, false)
    
    if IsPedInAnyVehicle(playerPed, false) then
        ESX.Game.DeleteVehicle(vehicle)
        TriggerEvent(Config.Notify, '¡Vehículo eliminado!')
    else
        TriggerEvent(Config.Notify, 'No estás en un vehículo.')
    end
end, false)
...
function SpawnVehicle(props)
  ...
  --  if cooldowns[playerId] and cooldowns[playerId][vehicleModel] then
   --     local currentTime = GetGameTimer()
    --    local cooldownEndTime = cooldowns[playerId][vehicleModel]

    --    if currentTime < cooldownEndTime then
     --       local remainingCooldown = math.floor((cooldownEndTime - currentTime) / 1000) 
      --      TriggerEvent("chatMessage", "^1[depo]", {255, 0, 0}, "Debes esperar " .. remainingCooldown .. " segundos antes de volver a spawnear este vehículo.")
      --      return
      --  end
   -- end
    ...
    ESX.Game.SpawnVehicle(props.model, garage.SpawnPosition, garage.SpawnPosition.w, function(vehicle)
        if DoesEntityExist(vehicle) then
            ...
            exports['LegacyFuel']:SetFuel(vehicle, 200)
            currentCar = vehicle
        end
    end)
    ...
    cooldowns[playerId][vehicleModel] = GetGameTimer() + 180000 
end
