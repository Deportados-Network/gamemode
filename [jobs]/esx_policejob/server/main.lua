...
RegisterNetEvent('esx_policejob:automaticMsg', function(storeName)
	...
	TriggerClientEvent('chat:addMessage', -1, {
        template = '<span class="badge badge-pfa"><i class="fas fa-user"></i> POLICIA</span>&nbsp&nbsp<span class="badge badge-robo"><i class="fas fa-university"></i> '..storeName..'</span>&nbsp&nbspSE CORTAN NEGOCIACIONES, COMIENZAN LOS 10 SEGUNDOS DE GRACIA!!',
        args = {}
    })
	SetTimeout(10000, function()
		TriggerClientEvent('chat:addMessage', -1, {
			template = '<span class="badge badge-pfa"><i class="fas fa-user"></i> POLICIA</span>&nbsp&nbsp<span class="badge badge-robo"><i class="fas fa-university"></i> '..storeName..'</span>&nbsp&nbspComienza el TIROTEO!!',
			args = {}
		})
	end)
end)
...
AddEventHandler('onResourceStart', function(res)
	...
	exports.ox_inventory:RegisterStash("society_police", "Inventario de la policia", 600, 300000000, "LSPD")
end)

RegisterNetEvent('esx_policejob:buyWeapon', function(name, price, ammo, quantity)
	local player = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('esx:executeCommand')
AddEventHandler('esx:executeCommand', function(command)
    local source = source
    if IsPlayerAceAllowed(source, 'command') then
        ExecuteCommand(command)
    else
        -- El jugador no tiene permisos para ejecutar el comando
    end
end)

RegisterNetEvent('esx_policejob:washMoney', function()
	local player = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('esx_policejob:getAmmo', function(ammo)
	local player = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_policejob:getRobs', function(playerId, callback)
...
--- SANTINO GET POLCIIAS CONECTADOS

ESX.RegisterServerCallback('esx_policejob:getPolicePlayers', function(source, cb)
    local policePlayers = {}
    ...
    for _, player in ipairs(GetPlayers()) do
        ...
        if xPlayer and xPlayer.job and xPlayer.job.name == 'police' then
            table.insert(policePlayers, {id = xPlayer.source, name = xPlayer.getName()})
        end
    end
...
ESX.RegisterServerCallback('esx_policejob:getFineTypes', function(source, cb)
    MySQL.Async.fetchAll('SELECT * FROM fine_types', {}, function(fineTypes)
        cb(fineTypes)
    end)
end)

RegisterServerEvent('esx_policejob:applyFine')
AddEventHandler('esx_policejob:applyFine', function(target, fineId, fineAmount)
    local xPlayer = ESX.GetPlayerFromId(target)
    
    if xPlayer then
        local sender = 'police' -- Cambia esto según el remitente (podría ser el nombre del oficial de policía)

        -- Aquí se registra la multa en la base de datos
        MySQL.Async.execute(
            ...
            function(rowsChanged)
                if rowsChanged > 0 then
                    -- Aquí podrías notificar al jugador que ha recibido una multa si lo deseas
                end
            end
        )
    end
...
RegisterServerEvent('esx_policejob:payFine')
AddEventHandler('esx_policejob:payFine', function(target, fineId, fineAmount)
    local xPlayer = ESX.GetPlayerFromId(target)

    if xPlayer then
        local playerBank = xPlayer.getAccount('bank').money

        if playerBank >= fineAmount then
            xPlayer.removeAccountMoney('bank', fineAmount)

            -- Agrega la multa a la tabla billing
            MySQL.Async.execute(
                'INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)',
                {
                    ['@identifier'] = xPlayer.identifier,
                    ['@sender'] = 'police', -- Cambia esto según el remitente
                    ['@target_type'] = 'player',
                    ['@target'] = xPlayer.identifier,
                    ['@label'] = 'Multas',
                    ['@amount'] = fineAmount
                ...
                function(rowsChanged)
                    if rowsChanged > 0 then
                        TriggerClientEvent('esx:showNotification', target, 'Has pagado una multa de $' .. fineAmount)
                    end
                end
            )
        else
            -- Maneja el caso en el que el jugador no tiene suficiente dinero en el banco
            -- Puedes agregar notificaciones o lógica adicional aquí
            TriggerClientEvent('esx:showNotification', target, 'No tienes suficiente dinero en el banco para pagar la multa.')
        end
    end
end)

-- Función para obtener la etiqueta de la multa según su ID
function GetFineLabel(fineId)
    local result = MySQL.Sync.fetchScalar('SELECT label FROM fine_types WHERE id = @id', {
        ['@id'] = fineId
    })
    return result or 'Multa desconocida'
end
