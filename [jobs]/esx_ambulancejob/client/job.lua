...
function OpenMobileAmbulanceActionsMenu()
    local elements = {
        { label = "Menú EMS", value = "citizen_interaction", icon = "fas fa-ambulance" }
    }
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
        ...
        if action == "citizen_interaction" then
            local elements2 = {
                { label = "Ambulancia", icon = "fas fa-ambulance" },
                { label = "Revivir", value = "revive", icon = "fas fa-syringe" },
                { label = "Pequeño", value = "small", icon = "fas fa-bandage" },
                { label = "Grande", value = "big", icon = "fas fa-bandage" },
                { label = "Meter en el vehículo", value = "put_in_vehicle", icon = "fas fa-car" },
                { label = "Buscar", value = "search", icon = "fas fa-syringe" },
            }

            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions_sub', {
                title = 'EMS Menu',
                align = 'right',
                elements = elements2
            }, function(data2, menu2)
                local subAction = data2.current.value

                if subAction == 'revive' then
                ...
                elseif subAction == 'big' then
                    -- Lógica para "big"
                    ESX.ShowNotification("Lógica para Big")
                elseif subAction == 'put_in_vehicle' then
                    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
...
RegisterCommand('screenfade', function()
...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			ESX.ShowNotification("Uniforme invalido")
		end
	end)
end
	...
	exports['ox_lib']:registerContext({
		...
		options = {
			...
			{title = "Uniforme", icon = "fas fa-tshirt", onSelect = function()
				setUniform('normal', playerPed)
			end}
		}
	})
	exports['ox_lib']:showContext("ambulance_wear")
end
...
function OpenPharmacyMenu()
	...
	ESX.OpenContext("right", elements, function(menu, element)
		local elements2 = {
			...
			{ title = "Cantidad", input = true, inputType = "number", inputMin = 1, inputMax = 100,
				inputPlaceholder = "Cantidad para comprar.." },
			{ icon = "fas fa-check-double", title = "Confirmar", val = "confirm" }
		}
