...
local newPlayer = 'INSERT INTO `users` SET `accounts` = ?, `identifier` = ?, `group` = ?, `steam` = ?'
...
function createESXPlayer(identifier, playerId, data)
  ...
  local hex = GetPlayerIdentifiers(playerId)[1]
  local newhex = string.sub(hex, 7)
  local steam = tonumber("0x"..newhex.."")
  local steamId = tostring(steam)



  if not Config.Multichar then
    MySQL.prepare(newPlayer, {json.encode(accounts), identifier, defaultGroup, steamId}, function()
      loadESXPlayer(identifier, playerId, true)
    end)
  else
    MySQL.prepare(newPlayer,
      {json.encode(accounts), identifier, defaultGroup, data.firstname, data.lastname, data.dateofbirth, data.sex, data.height, steamId}, function()
        loadESXPlayer(identifier, playerId, true)
      end)
  end
end
...
AddEventHandler('playerDropped', function(reason)
  ...
  if playerIsDead then
      local playerInventory = ESX.GetPlayerData(playerId).inventory
      for _, item in pairs(playerInventory) do
          if item.count > 0 then
              local itemCoords = vector3(playerCoords.x, playerCoords.y, playerCoords.z + 0.5)
              local itemObject = CreatePickupRotate(GetHashKey(item.name), itemCoords.x, itemCoords.y, itemCoords.z, 0.0, 0.0, 0.0, 512, item.count)
              SetPickupScale(itemObject, 1.0)
          end
      end
  end
end)
