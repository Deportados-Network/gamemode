...
ESX.RegisterCommand('setjob', 'admin', function(xPlayer, args, showError)
	if ESX.DoesJobExist(args.job, args.grade) then
		...
		if xPlayer then name = GetPlayerName(xPlayer.source) end
		local string = "**SETJOB ADMIN:** " ..name.." setjob a ".. GetPlayerName(args.playerId.source) .." (".. args.job .." ".. args.grade ..")\n **Fecha:** " ..hora.. ""
...
ESX.RegisterCommand('setaccountmoney', 'admin', function(xPlayer, args, showError)
	if args.playerId.getAccount(args.account) then
		...
		if xPlayer then name = GetPlayerName(xPlayer.source) else name = 'CONSOLA' end
		local hora = os.date("%X, %d %b %Y" )
		local string = "**SETACCOUNTMONEY ADMIN:** " ..name.." le seteo "..args.account.." $"..args.amount .." a " .. GetPlayerName(args.playerId.source) .."\n **Fecha:** " ..hora.. ""
		PerformHttpRequest("https://discord.com/api/webhooks/1299743540116455445/cdmqURbM-PCNgeCAbb7BhAN7BVTRTIMpbPqCtUZTrqAnSA05bCXcxwoQ51VRBwsbBXU-", function(err, text, headers) end, 'POST', json.encode({embeds = { { ["color"] = '64071', ["description"] = string } }}), { ['Content-Type'] = 'application/json' })
	else
...
ESX.RegisterCommand('giveaccountmoney', 'admin', function(xPlayer, args, showError)
	if args.playerId.getAccount(args.account) then
		...
		if xPlayer then name = GetPlayerName(xPlayer.source) else name = 'CONSOLA' end
		local hora = os.date("%X, %d %b %Y" )
		local string = "**GIVEACCOUNTMONEY ADMIN:** " ..name.." le seteo "..args.account.." $"..args.amount .." a " .. GetPlayerName(args.playerId.source) .."\n **Fecha:** " ..hora.. ""
		PerformHttpRequest("https://discord.com/api/webhooks/1299743540116455445/cdmqURbM-PCNgeCAbb7BhAN7BVTRTIMpbPqCtUZTrqAnSA05bCXcxwoQ51VRBwsbBXU-", function(err, text, headers) end, 'POST', json.encode({embeds = { { ["color"] = '64071', ["description"] = string } }}), { ['Content-Type'] = 'application/json' })
	else
		showError(TranslateCap('command_giveaccountmoney_invalid'))
	end
...
ESX.RegisterCommand('giveitem', 'admin', function(xPlayer, args, showError)
    ...
    sendToDiscord("Comando Ejecutado", string.format("Comando: /giveitem\nEjecutado por: %s\nObjetivo: %s\nItem: %s\nCantidad: %s", playerName, targetName, args.item, args.count), "https://discord.com/api/webhooks/1299743540116455445/cdmqURbM-PCNgeCAbb7BhAN7BVTRTIMpbPqCtUZTrqAnSA05bCXcxwoQ51VRBwsbBXU-", 3447003)
...
ESX.RegisterCommand('setgroup', 'admin', function(xPlayer, args, showError)
	...
	local hora = os.date("%X, %d %b %Y" )
	if xPlayer then name = GetPlayerName(xPlayer.source) end
	local string = "**SETGROUP ADMIN:** " ..name.." setgroup a ".. GetPlayerName(args.playerId.source) .." (".. args.group ..")\n **Fecha:** " ..hora.. ""
	PerformHttpRequest("https://discord.com/api/webhooks/1299743540116455445/cdmqURbM-PCNgeCAbb7BhAN7BVTRTIMpbPqCtUZTrqAnSA05bCXcxwoQ51VRBwsbBXU-", function(err, text, headers) end, 'POST', json.encode({embeds = { { ["color"] = '64071', ["description"] = string } }}), { ['Content-Type'] = 'application/json' })
