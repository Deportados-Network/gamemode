...
RegisterNetEvent("esx_identity:bucket", function(typeOf)
	local playerId = source
	if (typeOf == "playerId") then
		SetPlayerRoutingBucket(playerId, playerId)
	else
		SetPlayerRoutingBucket(playerId, 0)
		TriggerClientEvent('ox_lib:client:notify', playerId, "Registro", "Has sido devuelto a la dimensión 0, bienvenido a Deportados Roleplay.", "inform")
	end
end)
