...
--Functions
    ...
    RegisterNetEvent("esx_banking:interact", function()
        if (isNear) then
            BANK:HandleUi(true, true)
        else
            ESX.ShowNotification("No estás cerca de un cajero automático.", "error")
        end
    end)
    ...
    function BANK:TextUi(state, atm)
        ...
        uiActive = state
        if not state then
            return ox:hideTextUI()
        end
        ox:showTextUI('E - Abrir banco', {
            position = "top-center",
