	...
	ESX.UI.Menu.Open('default' , GetCurrentResourceName() , 'room' , {
		...
		if data.current.value == 'invite_player' then
			...
			ESX.UI.Menu.Open('default' , GetCurrentResourceName() , 'room_invite' , {
				...
				exports['mythic_notify']:DoCustomHudText('cajaverde' , ('Has invitado a '..GetPlayerName(data2.current.value)) , 7000)
			end , function(data2 , menu2)
				menu2.close()
			end)
		...
		elseif data.current.value == 'remove_cloth' then
			...
			ESX.TriggerServerCallback('esx_property:getPlayerDressing' , function(dressing)
				...
				ESX.UI.Menu.Open('default' , GetCurrentResourceName() , 'remove_cloth' , {
					...
					exports['mythic_notify']:DoCustomHudText('cajaverde' , ('El conjunto fue eliminado de tu armarío') , 7000)
				end , function(data2 , menu2)
					menu2.close()
				end)
			end)
...
function OpenPropertyInventoryMenu(property , owner)
	exports.ox_inventory:openInventory('stash', {id='house-'..owner, owner=owner})
