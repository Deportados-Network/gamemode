...
AddEventHandler('onResourceStart', function(res)
    if res == GetCurrentResourceName() then
		local query = MySQL.query.await('SELECT * FROM owned_properties')
		for i,v in pairs(query) do
            exports.ox_inventory:RegisterStash("house-"..v.owner, "Casa-"..v.owner, 100, 100000, v.owner)
		end
    end
end)
...
function SetPropertyOwned(name, price, rented, owner)
	MySQL.insert('INSERT INTO owned_properties (name, price, rented, owner) VALUES (?, ?, ?, ?)', {
		name,
		price,
		(rented and 1 or 0),
		owner
		...
		if xPlayer then
			exports.ox_inventory:RegisterStash("house-"..owner, "Casa-"..owner, 100, 100000, owner)
...
function RemoveOwnedProperty(name, owner, noPay)
	MySQL.query('SELECT id, rented, price FROM owned_properties WHERE name = ? AND owner = ?', {
		...
		if result[1] then
			MySQL.query('DELETE FROM owned_properties WHERE id = ?', {
				result[1].id
			}, function(rowsChanged)
				local xPlayer = ESX.GetPlayerFromIdentifier(owner)
...
AddEventHandler('esx_property:saveLastProperty', function(property)
	...
	MySQL.update('UPDATE users SET last_property = ? WHERE identifier = ?', {
		xPlayer.identifier,
		property
	})
end)
...
AddEventHandler('esx_property:deleteLastProperty', function()
	...
	MySQL.update('UPDATE users SET last_property = NULL WHERE identifier = ?', {
		xPlayer.identifier
	})
end)
...
ESX.RegisterServerCallback('esx_property:getOwnedProperties', function(source, cb)
	...
	MySQL.query('SELECT name, rented FROM owned_properties WHERE owner = ?', {
		xPlayer.identifier
	}, function(result)
		cb(result)
	end)
...
ESX.RegisterServerCallback('esx_property:getLastProperty', function(source, cb)
	...
	MySQL.query('SELECT last_property FROM users WHERE identifier = ?', {
		xPlayer.identifier
	}, function(users)
		...
		for k,v in ipairs(result) do
			table.insert(tasks, function(cb)
				...
				if xPlayer then
					...
					MySQL.query('SELECT accounts FROM users WHERE identifier = ?', {
						v.owner
					}, function(accounts)
						if accounts then
