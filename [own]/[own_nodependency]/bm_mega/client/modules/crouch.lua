...
-- Add the following function to make the player stand up when aiming while crouched
local function HandleAimWhileCrouched()
    while CrouchedForce do
        Citizen.Wait(0)

        if Crouched and IsPlayerFreeAimed() then
            NormalWalk()
            CrouchedForce = false
        end
    end
...
-- Modify your CrouchLoop function to include the new thread for handling aim while crouched
CrouchLoop = function()
    SetupCrouch()
    while CrouchedForce do
        ...
        if (IsPedArmed(ped, 4)) then
            DisablePlayerFiring(PlayerInfo.playerPed, true)
        else
            DisablePlayerFiring(PlayerInfo.playerPed, false)
        end

        local CanDo = CanCrouch()
        if CanDo and Crouched and IsPlayerFreeAimed() then
            SetPlayerAimSpeed()
            CreateThread(HandleAimWhileCrouched)  -- Check for aiming while crouched
